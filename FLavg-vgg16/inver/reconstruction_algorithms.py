...
class GradientReconstructor():
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), dryrun=False, eval=True, tol=None):
        """Reconstruct image from gradient."""#梯度重建图像
        start_time = time.time()
        if eval:
            self.model.eval()

        dm, ds,real_picture = self.mean_std

        stats = defaultdict(list)
        x = self._init_images(img_shape)  #随机图片
        ...
        if labels is None:
            ...
            assert labels.shape[0] == self.num_images  #1
            self.reconstruct_label = False
       ...
       # plt.figure(figsize=(10, 10))
        ...
        try:
            for trial in range(self.config['restarts']):
                x_trial, labels ,min_loss = self._run_trial(trial,x[trial], input_data, labels, dryrun=dryrun)
                # Finalize
                ...
                if self.config['scoring_choice'] in ['pixelmean', 'pixelmedian']:
                    ...
                    history_image.append(x_optimal.detach())
                   # history_image = x_optimal.detach()
                    ...
                    if trial == 0:
                        ...
                        plt.imshow(To_image((torch.clamp(history_image[0]* ds + dm, 0, 1))[0].cpu()))
                        #plt.title('The real picture')
                        plt.title("iter=%d" % (trial + 1))
                        #plt.savefig('./plt/' + 'The real picture' + '.png')
                        plt.savefig('./' + str(trial + 1) + '-' + 'label-' + str(labels.item()) + '-' + 'history_image' + '.png')
                    else:
                        plt.imshow(To_image((torch.clamp(history_image[trial-1] * ds + dm, 0, 1))[0].cpu()))
                        if trial == 9:
                            plt.imshow(To_image((torch.clamp(history_image[trial ] * ds + dm, 0, 1))[0].cpu()))
                            plt.title("iter=%d" % (trial+1))
                            plt.savefig('./' + str(trial+1) + '-' + 'label-' + str( labels.item()) + '-' + 'history_image' + '.png')
                        plt.title("iter=%d" % (trial+1))
                        #plt.axis('off')
                #plt.savefig('./plt/' + 'cifar' + 'history_image' + '.png')
                        plt.savefig('./' + str(trial+1) + '-' + 'label-' + str( labels.item()) + '-' + 'history_image' + '.png')
                        plt.show()
                    plt.clf()
                   ...
                   #  plt.title("iter=%d" % (trial+1))
                   # # plt.axis('off')
                   #  plt.savefig('./plt/' +str(trial+1) +'-' + 'label-'+str(labels.item()) + '-' + 'history_image' + '.png')
                   #  #plt.show()
                   #  plt.clf()
        ...
        #     #     pass
        #     # else:
        #     plt.imshow(To_image((torch.clamp(history_image[i ] * ds + dm, 0, 1))[0].cpu()))
        #     plt.title("iter=%d" % (i))
        #     plt.axis('off')
        #
        # plt.savefig('./plt/' + 'cifar' + 'history_image' + '.png')
        ...
        return history_image, stats,min_loss
    ...
    def _run_trial(self,k, x_trial, input_data, labels, dryrun=False):
        ...
        dm, ds,_ = self.mean_std
        if self.config['lr_decay']: #ture
        ...
        try:
            #for iteration in range(k*max_iterations+1,max_iterations*(k+1)+1,1):
            for iteration in range(0, max_iterations * (k+1)+1 ):
                ...
                with torch.no_grad():
                    ...
                    if iteration % 500 == 0:
                        ...
                        if min_loss > rec_loss.item():
                            min_loss = rec_loss.item()
                        X.append(iteration)
                        Y.append(rec_loss)
        ...
        plt.legend()  # 让图例生效
        plt.xticks(x, X, rotation=45)  # x坐标轴
        plt.margins(0)
        plt.subplots_adjust(bottom=0.15)  # 调整子图布局
        plt.xlabel(u"迭代次数")  # X轴标签
        plt.ylabel("loss")  # Y轴标签
        # plt.title("精确度")  # 标题
        plt.savefig('./plt/' +str(k+1)+ '-loss' + '.png')
        #plt.show()
        plt.clf()

        return x_trial.detach(), labels,min_loss
...
def reconstruction_costs(gradients, input_gradient, cost_fn='l2', indices='def', weights='equal'):
    ...
    ex = input_gradient[0]
    if weights == 'linear':  #weights=equal
